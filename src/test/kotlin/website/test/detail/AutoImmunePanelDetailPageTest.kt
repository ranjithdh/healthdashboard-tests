...
class AutoImmunePanelDetailPageTest {
    ...
    fun `should display page heading and description`() {
        val detail = AutoImmunePanelDetailPage(page).navigate() as AutoImmunePanelDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isANAMarkerVisible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isDsDNAMarkerVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isRAFactorMarkerVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isAntiCCPMarkerVisible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isJointHealthVisible()) { "food and dietConcern should be visible" }
        assert(detail.isChronicSymptomsVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isFamilyHistoryVisible()) { "drug and contactSensitivities should be visible" }
        assert(detail.isUnexplainedInflammationVisible()) { "drug and contactSensitivities should be visible" }
    }
