...
import website.page.detail.OmegaDetailPage
import kotlin.test.assertTrue


@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class OmegaDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = OmegaDetailPage(page).navigate() as OmegaDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isOmega3IndexVisible()) { "should display bacteria diversity" }
        assert(detail.isOmega6AndOmega3RatioVisible()) { "should display digestion and metabolism" }
        assert(detail.isPalmiticAcidIndexVisible()) { "should display immunity and inflammation" }
        assert(detail.isAAEPARatioVisible()){"should display disease and risk indicators" }
        assert(detail.isTransFatIndexTFIVisible()) { "should display mental health and personality" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isCardiovascularAndMetabolicRiskVisible()) { "food and dietConcern should be visible" }
        assert(detail.isBrainAndInflammationHealthVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isLifestyleAndNutritionVisible()) { "drug and contactSensitivities should be visible" }
