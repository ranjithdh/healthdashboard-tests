...
import website.page.detail.ThyroidHealthDetailPage


@TestInstance(TestInstance.Lifecycle.PER_CLASS)
@Epic(Modules.EPIC_WEBSITE)
class ThyroidHealthDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = ThyroidHealthDetailPage(page).navigate() as ThyroidHealthDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isThyroidStimulatingHormoneTSHVisible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isTotalThyroxineT4Visible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isFreeThyroxine_fT4Visible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isTotalTriiodothyronineT3Visible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isFreeTriiodothyronine_fT3Visible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isT3_T4RatioVisible()) { "ThyroidStimulatingHormone should visible" }
        assert(detail.isFreeT3_Free_T4_RatioVisible()) { "ThyroidStimulatingHormone should visible" }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isEnergyAndMetabolismVisible()) { "food and dietConcern should be visible" }
        assert(detail.isMoodAndHormoneBalanceVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isFamilyAndRiskFactorsVisible()) { "drug and contactSensitivities should be visible" }
