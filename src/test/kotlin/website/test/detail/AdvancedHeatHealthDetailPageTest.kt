...
class AdvancedHeatHealthDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = AdvancedHeartHealthDetailPage(page).navigate() as AdvancedHeartHealthDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isLipidProfileAndRatiosVisible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isAdvancedLipoproteinsVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isApolipoproteinsVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isInflammationMarkersVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isMetabolicMarkerVisible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isAtRiskIndividualsVisible()) { "food and dietConcern should be visible" }
        assert(detail.isPerformanceAndLongevitySeekersVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isInflammationAndRiskMonitoringVisible()) { "drug and contactSensitivities should be visible" }
    }
