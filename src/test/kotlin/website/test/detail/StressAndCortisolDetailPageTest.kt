...
class StressAndCortisolDetailPageTest {
    ...
    fun `should display page heading and description`() {
        val detail = StressAndCortisolDetailPage(page).navigate() as StressAndCortisolDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isCortisolAwakeningResponse_CAR_Visible()) { "should display bacteria diversity" }
        assert(detail.isMidMorningDeclineVisible()) { "should display digestion and metabolism" }
        assert(detail.isAfternoonRecoveryVisible()) { "should display immunity and inflammation" }
        assert(detail.isEveningRhythmVisible()){"should display disease and risk indicators" }
        assert(detail.isOverallCurveVisible()){"should display disease and risk indicators" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isStressAndEnergyIssuesVisible()) { "food and dietConcern should be visible" }
        assert(detail.isMoodAndMentalHealthVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isPreventiveHealthVisible()) { "drug and contactSensitivities should be visible" }
    ...
    fun `should display what to expect content`() {
        ...
        assert(detail.isSendForProcessingVisible()) { "sample collection should be visible" }
