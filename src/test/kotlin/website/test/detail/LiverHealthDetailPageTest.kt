...
class LiverHealthDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = LiverHealthDetailPage(page).navigate() as LiverHealthDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isSGOT_SGPT_RatioVisible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isFattyLiverIndexFLIVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isAlbuminGlobulinAGRatioVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isGammaGlutamylTransferaseGGTVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isAlkalinePhosphataseALPVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isAlanineTransaminaseSGPTVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isAspartateAminotransferaseSGOTVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isSerumGlobulinVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isSerumAlbuminVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isTotalBilirubinVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isBilirubinDirectVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isBilirubinIndirectVisible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isLifestyleRisksVisible()) { "food and dietConcern should be visible" }
        assert(detail.isPreventiveHealthVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isSymptomsAndWarningSignsVisible()) { "drug and contactSensitivities should be visible" }
    }
