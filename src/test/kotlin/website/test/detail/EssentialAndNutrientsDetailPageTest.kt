...
class EssentialAndNutrientsDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = EssentialAndNutrientsDetailPage(page).navigate() as EssentialAndNutrientsDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isVitaminB12Visible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isVitaminDVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isIronVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isFerritinVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isHomocysteineVisible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isEnergyAndVitalityVisible()) { "food and dietConcern should be visible" }
        assert(detail.isPreventiveHealthVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isSpecificHealthConcernsVisible()) { "drug and contactSensitivities should be visible" }
    }
