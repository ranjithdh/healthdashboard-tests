...
class WomenHealthDetailPageTest {
    ...
    fun `should display page heading and description`() {
        val detail = WomenHealthDetailPage(page).navigate() as WomenHealthDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isFSHFollicleStimulatingHormoneVisible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isLHLuteinizingHormoneVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isEstradiolDay2Visible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isCycleIrregularitiesVisible()) { "food and dietConcern should be visible" }
        assert(detail.isFertilityPlanningVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isHormoneBalanceVisible()) { "drug and contactSensitivities should be visible" }
