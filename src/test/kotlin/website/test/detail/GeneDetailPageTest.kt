...
class GeneDetailPageTest : BaseTest() {
    ...
    fun `should display page heading and description`() {
        val detail = GeneDetailPage(page).navigate() as GeneDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isNutritionAndMetabolismVisible()) { "should display bacteria diversity" }
        assert(detail.isFitnessAndPerformanceVisible()) { "should display digestion and metabolism" }
        assert(detail.isSkinHairAndAppearanceVisible()) { "should display immunity and inflammation" }
        assert(detail.isSleepAndLifestyleVisible()){"should display disease and risk indicators" }
        assert(detail.isMentalHealthAndPersonalityVisible()) { "should display mental health and personality" }
        assert(detail.isCardioAndMetabolicHealthVisible()) { "should display cardio and health" }
        assert(detail.isImmuneAndInflammatoryHealthVisible()) { "should display immunity and inflammation" }
        assert(detail.isDigestiveHealthVisible()) { "should display digestion and metabolism" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isNutritionAndLifestyleGoalsVisible()) { "food and dietConcern should be visible" }
        assert(detail.isPreventiveHealthAndLongevity()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isMentalHealthAndPersonalityWhoShouldTakeVisible()) { "drug and contactSensitivities should be visible" }
        assert(detail.isFamilyPlanningAndRiskAwarenessVisible()){"should display familyPlanningAndRiskAwareness and contactSensitivity" }
    ...
    fun `should display what to expect content`() {
        ...
        assert(detail.isSendForProcessingVisible()) { "send to processing should be visible" }
