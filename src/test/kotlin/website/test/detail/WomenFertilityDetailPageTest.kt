...
class WomenFertilityDetailPageTest {
    ...
    fun `should display page heading and description`() {
        val detail = WomenFertilityDetailPage(page).navigate() as WomenFertilityDetailPage
    ...
    fun `should display what is measured section content`() {
        ...
        assert(detail.isWhtMeasuredDescriptionVisible()) { "FSHFollicleStimulatingHormone should be visible" }
        assert(detail.isAMHMarkerVisible()) { "LHLuteinizing hormone should be visible" }
        assert(detail.isProlactinMarkerVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isFSHMarkerVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isLHMarkerVisible()) { "Estradiol day2 should be visible" }
        assert(detail.isEstradiolDay2MarkerVisible()) { "Estradiol day2 should be visible" }
    }
    ...
    fun `should display who should take this test content`() {
        ...
        assert(detail.isCycleHealthVisible()) { "food and dietConcern should be visible" }
        assert(detail.isOvarianReserveVisible()) { "environment and seasonalAllergens should be visible" }
        assert(detail.isOvulationAndHormoneBalanceVisible()) { "drug and contactSensitivities should be visible" }
        assert(detail.isMetabolicHealthVisible()) { "drug and contactSensitivities should be visible" }
    }
