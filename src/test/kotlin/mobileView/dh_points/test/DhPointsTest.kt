...
import mobileView.dh_points.page.DhPointsPage
...
class DhPointsTest : BaseTest() {
    ...
    private lateinit var dhPointsPage: DhPointsPage
    ...
    fun createContext() {
        ...
        dhPointsPage = DhPointsPage(page)
    ...
    @Test
    @Story("DH Points – Full Signup and Pipeline Flow with API")
    fun `dh points full flow until calendly`() {
        // Step 1: Generate dynamic test user
        logger.info { "═══════════════════════════════════════" }
        logger.info { "  TEST: dh points full flow test" }
        logger.info { "═══════════════════════════════════════" }
        StepHelper.step(DH_POINTS_GENERATE_USER)
        var dynamicMobileNumber = "9" + (System.currentTimeMillis() % 1000000000).toString().padEnd(9, '0')
        dynamicMobileNumber = "9128821145"
        val testUser = TestConfig.TestUsers.DH_POINTS_GENERATE_USER.copy(mobileNumber = dynamicMobileNumber)
...
//         Step 6: Verify blood test card
        ...
        dhPointsPage.triggerBloodDataPipeline(
            stgUserId = TestConfig.USER_ID,
            prdUserId = TestConfig.DH_POINTS_PROD_USER_ID
        )

        homePage.refreshPage()
            .claimYourConsultCard()
        homePage.consultationConfirmationCard()
        ...
        // Step 11: Navigate home and proceed to expert consultation
...
//        homePage.consultationWithExpertCard()
//        logger.info { "═══════════════════════════════════════" }
//        logger.info { "  TEST COMPLETE: Consultation with expert" }
//        logger.info { "═══════════════════════════════════════" }
//
...
//        logger.info { "═══════════════════════════════════════" }
    ...
    @Test
    @Story("DH Points – after calendly")
    fun `dh points flow after calendly`() {
        val loginPage = LoginPage(page).navigate() as LoginPage
...
//        var dynamicMobileNumber = "9" + (System.currentTimeMillis() % 1000000000).toString().padEnd(9, '0')
//        val testUser = TestConfig.TestUsers.DH_POINTS_GENERATE_USER.copy(mobileNumber = dynamicMobileNumber)
//        dynamicMobileNumber = "9126980203"
//        logger.info { "[STEP] Dynamic Mobile Number: $dynamicMobileNumber" }
...
//
//
//        dhPointsPage.triggerBloodDataPipeline(stgUserId = TestConfig.USER_ID, prdUserId = TestConfig.DH_POINTS_PROD_USER_ID)
////        triggerDataPipeline(TestConfig.USER_ID)
//
//        // Step 10:
//        val homePage2 = HomePage(page).navigate() as HomePage
//        homePage2.claimYourConsultCard()
...
//        StepHelper.step(DH_POINTS_CONSULT_WITH_EXPERT)
//        logger.info { "[STEP] Navigating to home and proceeding to consult with expert..." }
//        val homePage3 = HomePage(page).navigate() as HomePage
//        homePage3.claimYourConsultCard()
////            .consultationWithExpertCard()
//        logger.info { "═══════════════════════════════════════" }
//        logger.info { "  TEST COMPLETE: Consultation with expert" }
//        logger.info { "═══════════════════════════════════════" }
//
//        homePage3.rewardPointsValidation()
//        logger.info { "═══════════════════════════════════════" }
//        logger.info { "  TEST COMPLETE: dh points verification" }
//        logger.info { "═══════════════════════════════════════" }
//
...
//
//    @Test
//    @Story("DH Points Test")
//    fun `dh points verification until Questionnair`() {
//
//    }
//
//    // ─────────────────────────────────────────────────────────────
